...
async def async_crawl(url, session, method='GET', proxy=None, data=None, headers=None, encoding=None):
    ...
    try:
        ...
        response = Response(create_time=int(time.time() * 1000), status=r.status, url=url, html=data, is_valid=False)
...
async def async_crawl_and_check(url, session, pattern, method='GET', proxy=None, data=None, headers=None,
    ...
    if response is None:
        ...
        html, status = response.html, response.status
    is_valid = await validate.check_response(url, pattern, proxy, status, html, valid_length, xpath, value)
    if is_valid:
...
async def crawl(url, proxies, pattern=None, method='GET', data=None, headers=None, valid_length=None,
    ...
    async with aiohttp.ClientSession() as session:
        if need_check:
            tasks = [asyncio.ensure_future(async_crawl_and_check(url, session, pattern, method, proxy, data, headers,
                                                                 valid_length, xpath, value)) for proxy in proxies]
        else:
            tasks = [asyncio.ensure_future(async_crawl(url, session, method, proxy, data, headers))
