...
def status_checker(pattern, status):
...
def html_checker(html, valid_length, xpath, value):
    if html is None or len(html) < valid_length or any((black_keyword in html
                                                        for black_keyword in config.BLACK_KEYWORDS)):
        return False
    if xpath == 'keyword' and value not in html:
        return False
...
async def _result_saver(key, url, status, html, is_valid, redis):
    max_count = config.RESULT_SAVE_NUM
    ...
    await redis.ltrim(result_key, 0, max_count)
...
async def check_response(url, pattern, proxy, status, html, valid_length, xpath, value):
    ...
    is_valid = all((status_checker(pattern, status), html_checker(html, valid_length, xpath, value),))
